...
The Toy Data Simulation Experiment folder, which is within the Simulations Experiments folder, includes figures and results for OMASGAN evaluated on synthetic data. The paper 'OMASGAN: Out-of-Distribution Minimum Anomaly Score GAN for Sample Generation on the Boundary' in Section 5.1 and in Figure 3 presents the evaluation of OMASGAN on synthetic data for a multimodal distribution with disconnected components for p<sub>**x**</sub> (Author, 2021).

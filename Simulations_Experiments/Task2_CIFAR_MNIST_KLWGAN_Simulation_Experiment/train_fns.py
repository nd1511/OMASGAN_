...
def select_loss(config):
    if config['loss_type'] == 'hinge':
        ...
        def gen_f(x, rreal_samples2, rz_, ry_):
            return losses.loss_kl_gen(x, rreal_samples2, rz_, ry_, temp)
        return dis_f, gen_f
        ...
        raise ValueError('The loss not defined.')
    ...
    def train(x, y):
        ...
        for accumulation_index in range(config['num_G_accumulations']):
            ...
            G_loss = generator_loss(
                D_fake, real_samples2, z_, G.forward(z_, y_)) / float(config['num_G_accumulations'])
            G_loss.backward()

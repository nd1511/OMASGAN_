    ...
    def train(x, y):
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                #D_fake, D_real, Ggg1g1G1g1G1 = GD(z_[:config['batch_size']], y_[:config['batch_size']], real_samples,
                #                    y_counter, train_G=False, return_G_z=True, split_D=config['split_D'])

                #_, Ggg1g1G1g1G1 = GD(z_[:config['batch_size']], y_[:config['batch_size']],
                #                   train_G=False, return_G_z=True, split_D=config['split_D'])
                ...
                # z_, y_ = prepare_z_y(batchsize, nrand, 1, device=device, fp16=False)
                # z_.sample_()
                #
                # y_.sample_()
                # y_.zero_()
                # y_counter = torch.zeros_like(y_).to(y_.device).long()
                # _, vreal = GD(z_[:xreal.shape[0]], y_[:xreal.shape[0]], xreal,
                #                    y_counter, train_G=False, split_D=False)
                #
                # Treal = self.conj.T(vreal)  # Mapped to T-space
                #
                # vmodel, xmodel = GD(z_, y_, train_G=True, return_G_z=True, split_D=False)
                #
                ...
                _, firstGgg1g1G1g1G1 = GD3(z_[:config['batch_size']], y_[:config['batch_size']],
                                     train_G=False, return_G_z=True, split_D=config['split_D'])

                # _, secondGgg1g1G1g1G1 = GD2(z_[:config['batch_size']], y_[:config['batch_size']],
                #                           train_G=False, return_G_z=True, split_D=config['split_D'])

                # dis_fake2 is B
                # dis_real2 is G

                #D_loss = discriminator_loss(D_fake, D_real, firstGgg1g1G1g1G1, secondGgg1g1G1g1G1)

                D_loss = discriminator_loss(D_fake, D_real, firstGgg1g1G1g1G1, firstGgg1g1G1g1G1)

                #D_loss = discriminator_loss(D_fake, D_real, Ggg1g1G1g1G1, Ggg1g1G1g1G1)
                #D_loss = discriminator_loss(D_fake, D_real, firstGgg1g1G1g1G1, secondGgg1g1G1g1G1)
                ...
                #D_loss_real, D_loss_fake = discriminator_loss(D_fake, D_real, Ggg1g1G1g1G1, Ggg1g1G1g1G1)
                #D_loss = discriminator_loss(D_fake, D_real, Ggg1g1G1g1G1, Ggg1g1G1g1G1)
        ...
        for accumulation_index in range(config['num_G_accumulations']):
            ...
            #D_fake, gvhgzagaGenerator = GD(z_, y_, train_G=True, return_G_z=True, split_D=config['split_D'])
            ...
            _, fiFirstgvhgzagaGenerator = GD3(z_[:config['batch_size']], y_[:config['batch_size']],
                                       train_G=False, return_G_z=True, split_D=config['split_D'])

            seSecgvhgzagaGenerator = x[counter2]

            # _, thThigvhgzagaGenerator = GD2(z_[:config['batch_size']], y_[:config['batch_size']],
            #                             train_G=False, return_G_z=True, split_D=config['split_D'])

            # G_loss = generator_loss(D_fake, fiFirstgvhgzagaGenerator, seSecgvhgzagaGenerator, thThigvhgzagaGenerator) / float(
            #     config['num_G_accumulations'])

            # G_loss = generator_loss(D_fake, fiFirstgvhgzagaGenerator, seSecgvhgzagaGenerator,
            #                         thThigvhgzagaGenerator) / float(
            #     config['num_G_accumulations'])
            ...
            #G_loss = generator_loss(D_fake, gvhgzagaGenerator, gvhgzagaGenerator, gvhgzagaGenerator) / float(config['num_G_accumulations'])

            #G_loss = generator_loss(D_fake, gvhgzagaGenerator) / float(config['num_G_accumulations'])

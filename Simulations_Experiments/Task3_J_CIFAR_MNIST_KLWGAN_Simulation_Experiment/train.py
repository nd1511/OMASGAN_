...
def run(config):
    ...
    utils.load_weights(G, D, state_dict, '../../weights', 'C10U_kl_5', 'best0', G_ema if config['ema'] else None)
    utils.load_weights(G3, D3, state_dict, '../weights', 'C10U_kl_5', 'best0', G_ema if config['ema'] else None)
    if config['parallel']:

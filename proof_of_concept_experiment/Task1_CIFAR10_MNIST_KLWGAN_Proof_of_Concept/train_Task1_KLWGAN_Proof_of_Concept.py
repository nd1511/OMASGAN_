...
def run(config):
    ...
    device = 'cuda'
    utils_Task1_KLWGAN_Proof_of_Concept.seed_rng(config['seed'])
    utils_Task1_KLWGAN_Proof_of_Concept.prepare_root(config)
    torch.backends.cudnn.benchmark = True
    model = __import__(config['model'])
    experiment_name = (config['experiment_name'] if config['experiment_name']
                       else utils_Task1_KLWGAN_Proof_of_Concept.name_from_config(config))
    ...
    if config['ema']:
        ...
        ema = utils_Task1_KLWGAN_Proof_of_Concept.ema(G, G_ema, config['ema_decay'], config['ema_start'])
    else:
        G_ema, ema = None, None
    if config['G_fp16']:
        G = G.half()
    ...
    if config['resume']:
        utils_Task1_KLWGAN_Proof_of_Concept.load_weights(G, D, state_dict, config['weights_root'], experiment_name, config['load_weights'] if config['load_weights'] else None, G_ema if config['ema'] else None)
    if config['parallel']:
    ...
    test_log = utils_Task1_KLWGAN_Proof_of_Concept.MetricsLogger(test_metrics_fname,
                                   reinitialize=(not config['resume']))
    print('Training Metrics will be saved to {}'.format(train_metrics_fname))
    train_log = utils_Task1_KLWGAN_Proof_of_Concept.MyLogger(train_metrics_fname,
                               reinitialize=(not config['resume']),
                               logstyle=config['logstyle'])
    utils_Task1_KLWGAN_Proof_of_Concept.write_metadata(config['logs_root'],
                         experiment_name, config, state_dict)
    D_batch_size = (config['batch_size'] * config['num_D_steps']
                    * config['num_D_accumulations'])
    loaders = utils_Task1_KLWGAN_Proof_of_Concept.get_data_loaders(**{**config, 'batch_size': D_batch_size,
                                        'start_itr': state_dict['itr']})
    G_batch_size = max(config['G_batch_size'], config['batch_size'])
    z_, y_ = utils_Task1_KLWGAN_Proof_of_Concept.prepare_z_y(G_batch_size, G.dim_z, config['n_classes'],
                               device=device, fp16=config['G_fp16'])
    fixed_z, fixed_y = utils_Task1_KLWGAN_Proof_of_Concept.prepare_z_y(G_batch_size, G.dim_z,
    ...
    sample = functools.partial(utils_Task1_KLWGAN_Proof_of_Concept.sample,
    ...
    for epoch in range(state_dict['epoch'], config['num_epochs']):
        if config['pbar'] == 'mine':
            pbar = utils_Task1_KLWGAN_Proof_of_Concept.progress(
        ...
        for i, (x, y) in enumerate(pbar):
            ...
            if (config['sv_log_interval'] > 0) and (not (state_dict['itr'] % config['sv_log_interval'])):
                train_log.log(itr=int(state_dict['itr']),
                              **{**utils_Task1_KLWGAN_Proof_of_Concept.get_SVs(G, 'G'), **utils_Task1_KLWGAN_Proof_of_Concept.get_SVs(D, 'D')})
            if config['pbar'] == 'mine':
            ...
            if (not (state_dict['itr'] % config['test_every'])) and (epoch >= config['start_eval']):
                if config['G_eval_mode']:
                    ...
                    if config['ema']:
                        G_ema.eval()
                utils_Task1_KLWGAN_Proof_of_Concept.sample_inception(
        ...
        state_dict['epoch'] += 1
def main():
    parser = utils_Task1_KLWGAN_Proof_of_Concept.prepare_parser()
